<?php
/**
 * Skyrocket Customizer Custom Controls - Edited by BUILDER for additional features
 *
 */
...
if ( class_exists( 'WP_Customize_Control' ) ) {
	...
	/**
	 * Text Radio Button Custom Control - BUILDER ADDED BG COLOR
	 *
	 * @author Anthony Hortin <http://maddisondesigns.com>
	 * @license http://www.gnu.org/licenses/gpl-2.0.html
	 * @link https://github.com/maddisondesigns
		...
		?>
			<div class="text_radio_button_control">
				...
				<div class="radio-buttons">
					<?php foreach ( $this->choices as $key => $value ) { ?>
            <?php
               // BUILDER check for color classes
               $us_b_span_class = '';
               $us_b_span_style = '';
               if(strstr($value, ' [.bg-primary]')){ $us_b_span_class = ' class="bg-primary"'; $value = str_replace(' [.bg-primary]', '', $value); }
               if(strstr($value, ' [.bg-secondary]')){ $us_b_span_class = ' class="bg-secondary"'; $value = str_replace(' [.bg-secondary]', '', $value); }
               if(strstr($value, ' [.bg-success]')){ $us_b_span_class = ' class="bg-success"'; $value = str_replace(' [.bg-success]', '', $value); }
               if(strstr($value, ' [.bg-info]')){ $us_b_span_class = ' class="bg-info"'; $value = str_replace(' [.bg-info]', '', $value); }
               if(strstr($value, ' [.bg-warning]')){ $us_b_span_class = ' class="bg-warning"'; $value = str_replace(' [.bg-warning]', '', $value); }
               if(strstr($value, ' [.bg-danger]')){ $us_b_span_class = ' class="bg-danger"'; $value = str_replace(' [.bg-danger]', '', $value); }
               if(strstr($value, ' [.bg-light]')){ $us_b_span_class = ' class="bg-light"'; $value = str_replace(' [.bg-light]', '', $value); }
               if(strstr($value, ' [.bg-dark]')){ $us_b_span_class = ' class="bg-dark"'; $value = str_replace(' [.bg-dark]', '', $value); }
               // Curved corners
               if($this->id == 'understrap_builder_buttons_curved_borders'){
                 $us_b_span_class = ' class="us_b_curved_corners"';
                 $us_b_span_style = ' style="border-radius: '.$key.'!important;"';
               }
            ?>
						<label class="radio-button-label">
							<input type="radio" name="<?php echo esc_attr( $this->id ); ?>" value="<?php echo esc_attr( $key ); ?>" <?php $this->link(); ?> <?php checked( esc_attr( $key ), $this->value() ); ?>/>
							<span<?php echo $us_b_span_class.$us_b_span_style; ?>><?php echo esc_attr( $value ); ?></span>
						</label>
					<?php	} ?>
				</div>
			</div>
      ...
      $input_attrs = $this->input_attrs;
		?>
			<div class="image_radio_button_control">
				...
				<?php foreach ( $this->choices as $key => $value ) { ?>
					<label class="radio-button-label<?php if(isset($input_attrs['tooltip'])){ ?> us_b_tipr<?php } ?>">
						<input type="radio" name="<?php echo esc_attr( $this->id ); ?>" value="<?php echo esc_attr( $key ); ?>" <?php $this->link(); ?> <?php checked( esc_attr( $key ), $this->value() ); ?>/>
						<?php if(isset($input_attrs['tooltip'])){ ?>
            <div class="builder_img_radio_parent">
              <img src="<?php echo esc_attr( $value['image'] ); ?>" alt="<?php echo esc_attr( $value['name'] ); ?>" title="<?php echo esc_attr( $value['name'] ); ?>" class="has-tooltip" 
                   data-tip="<?php echo esc_attr( $value['name'] ); ?>" data-mode="<?php echo esc_attr($input_attrs['tooltip']); ?>" />
            </div>
            <?php } else { ?>
            <img src="<?php echo esc_attr( $value['image'] ); ?>" alt="<?php echo esc_attr( $value['name'] ); ?>" title="<?php echo esc_attr( $value['name'] ); ?>" />
            <?php } ?>
